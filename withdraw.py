...
class Withdraw:
    def __init__(self, parent, accNum):
        ...
        self.label = ttk.Label(self.window, text="Enter amount to withdraw:")
        ...
        btn_login = ttk.Button(self.window, text="Withdraw", command=self.handleWithdraw)
    ...
    def handleWithdraw(self):
        ...
        if amount.isdigit():
            if float(amount) >= 1000:
                self.saveWithdraw(float(amount))
            else:
    ...
    def saveWithdraw(self, amount):
        ...
        if account_found:
            try:
                ...
                messagebox.showinfo("Success", f"Successfully withdrawed {amount} to account {self.accNum}.")
